...
Backup-Registry -path "HKCU\Control Panel\Desktop" -backupFile "Desktop_Backup.reg"
...
Backup-Registry -path $startupPath -backupFile "Run_Backup.reg"
Remove-ItemProperty -Path $startupPath -Name "OneDrive" -ErrorAction SilentlyContinue

# Melhorar o tempo de inicialização
Backup-Registry -path "HKLM\SYSTEM\CurrentControlSet\Control" -backupFile "Control_Backup.reg"
...
Backup-Registry -path $prefetchPath -backupFile "Prefetch_Backup.reg"
